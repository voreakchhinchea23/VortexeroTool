...
import { Sparkles, Search, Zap, Shield, Globe, Terminal, Tv, ExternalLink } from 'lucide-react';
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <div className="space-y-6 mb-8">
      {/* Main Hero Card */}
      <div
        className={`relative overflow-hidden rounded-3xl border p-6 sm:p-10 backdrop-blur-xl transition-all duration-300 ${
          isVibrant
            ? 'bg-gradient-to-br from-[#0e1322] via-[#12182b] to-[#1a1329] border-indigo-500/30 shadow-2xl shadow-indigo-500/10'
            : 'bg-gradient-to-br from-brand-900/10 via-indigo-900/5 to-purple-900/10 dark:from-brand-950/40 dark:via-indigo-950/20 dark:to-purple-950/30 border-brand-500/20'
        }`}
      >
        {/* Decorative gradient glow blobs */}
        <div
          className={`absolute -top-24 -right-24 w-80 h-80 rounded-full blur-3xl pointer-events-none ${
            isVibrant ? 'bg-fuchsia-500/20' : 'bg-brand-500/20'
          }`}
        />
        <div
          className={`absolute -bottom-24 -left-24 w-80 h-80 rounded-full blur-3xl pointer-events-none ${
            isVibrant ? 'bg-cyan-500/20' : 'bg-indigo-500/20'
          }`}
        />
        ...
        <div className="relative z-10 max-w-3xl">
          <div
            className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold uppercase tracking-wider mb-4 ${
              isVibrant
                ? 'bg-gradient-to-r from-cyan-500/15 via-indigo-500/15 to-fuchsia-500/15 border-indigo-500/30 text-cyan-300'
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-3.5 rounded-2xl border text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none text-sm sm:text-base transition-all ${
                  isVibrant
                    ? 'bg-[#0b0f19]/90 border-slate-700/80 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/40 shadow-inner'
                    : 'bg-white/90 dark:bg-slate-900/90 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-brand-500 shadow-lg shadow-brand-500/5'
                }`}
              />
            </div>
            ...
            <button
              ...
              type="button"
              className={`px-5 py-3.5 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 text-white transition-all cursor-pointer shrink-0 active:scale-95 ${
                isVibrant
                  ? 'bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:to-pink-500 shadow-lg shadow-indigo-500/25'
                  : 'bg-slate-900 hover:bg-slate-800 dark:bg-brand-600 dark:hover:bg-brand-700 shadow-lg shadow-slate-950/10 dark:shadow-brand-500/20'
              }`}
            >
              <Terminal size={16} />
